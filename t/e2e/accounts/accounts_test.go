...
func (s *AccountsTestSuite) TestAccountsList() {
	...
	err = s.Backend.SelectAccount(accountInfo.WalletKeyInfo.Address, TestConfig.Account1.Password)
	...
	s.Equal(accounts[0].Hex(), accountInfo.WalletKeyInfo.Address,
		fmt.Sprintf("main account is not retured as the first key: got %s, expected %s", accounts[0].Hex(), "0x"+accountInfo.WalletKeyInfo.Address))
	...
	s.Equal(accounts[0].Hex(), accountInfo.WalletKeyInfo.Address, "main account is not retured as the first key")
...
func (s *AccountsTestSuite) TestCreateChildAccount() {
	...
	accountInfo, err := s.Backend.AccountManager().CreateAccount(TestConfig.Account1.Password)
	s.NoError(err)
	s.T().Logf("Account created: {address: %s, key: %s, mnemonic:%s}", accountInfo.WalletKeyInfo.Address, accountInfo.WalletKeyInfo.PubKey, accountInfo.Mnemonic)

	acct, err := account.ParseAccountString(accountInfo.WalletKeyInfo.Address)
...
func (s *AccountsTestSuite) TestRecoverAccount() {
	...
	addressCheck, pubKeyCheck, err := s.Backend.AccountManager().RecoverAccount(TestConfig.Account1.Password, accountInfo.Mnemonic)
	s.NoError(err, "recover acc failed")
	s.False(accountInfo.WalletKeyInfo.Address != addressCheck || accountInfo.WalletKeyInfo.PubKey != pubKeyCheck, "incorrect accound details recovered")

	// now test recovering, but make sure that acc/key file is removed i.e. simulate recovering on a new device
	acc, err := account.ParseAccountString(accountInfo.WalletKeyInfo.Address)
	...
	s.False(accountInfo.WalletKeyInfo.Address != addressCheck || accountInfo.WalletKeyInfo.PubKey != pubKeyCheck,
...
func (s *AccountsTestSuite) TestSelectAccount() {
	...
	s.NoError(err)
	s.T().Logf("Account created: {address: %s, key: %s}", accountInfo1.WalletKeyInfo.Address, accountInfo1.WalletKeyInfo.PubKey)

	accountInfo2, err := s.Backend.AccountManager().CreateAccount(TestConfig.Account1.Password)
	s.NoError(err)
	s.T().Logf("Account created: {address: %s, key: %s}", accountInfo2.WalletKeyInfo.Address, accountInfo2.WalletKeyInfo.PubKey)
	...
	err = s.Backend.SelectAccount(accountInfo1.WalletKeyInfo.Address, TestConfig.Account1.Password)
	s.NoError(err)
...
func (s *AccountsTestSuite) TestSelectedAccountOnRestart() {
	...
	s.Equal(selectedAccount.Address.Hex(), accountInfo2.WalletKeyInfo.Address, "incorrect address selected")
